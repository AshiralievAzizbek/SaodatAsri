...
public class DownloadTracker {
    ...
    public boolean isDownloaded(Uri mediaItemUri) {
        @Nullable Download download = downloads.get(mediaItemUri);
        return download != null && download.state != Download.STATE_FAILED;
    }
    ...
    public void toggleDownload(
        ...
        if (download != null && download.state != Download.STATE_FAILED) {
            DownloadService.sendRemoveDownload(
                    context, MyDownloadService.Companion.getJavaClass(), download.request.id, /* foreground= */ true);
        } else {
    ...
    private final class StartDownloadDialogHelper
        ...
        private void startDownload(DownloadRequest downloadRequest) {
            DownloadService.sendAddDownload(
                    context, MyDownloadService.Companion.getJavaClass(), downloadRequest, /* foreground= */ false);
        }

        private DownloadRequest buildDownloadRequest() {
            return downloadHelper
                    .getDownloadRequest(
                            Util.getUtf8Bytes(checkNotNull(Objects.requireNonNull(mediaItem.mediaMetadata.title).toString())))
